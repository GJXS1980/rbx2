...
def setup_task_environment(self):
    ...
    # 导入yaml文件
    data = (yaml.safe_load(open('/home/gjxs/ws/src/rbx2/rbx2_tasks/src/rbx2_tasks/test.yaml'))) 

    # 设置低电量的阈值(between 0 and 100)
    self.low_battery_threshold = rospy.get_param('~low_battery_threshold', 50)

    # 每个waypoint超时时间
    self.move_base_timeout = rospy.get_param("~move_base_timeout", 180) #seconds
